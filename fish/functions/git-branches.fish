...
function gb --description "Fuzzy find Git branches with advanced preview and actions"
    ...
    fzf --ansi \
        ...
        --bind "ctrl-d:execute(
            ...
            set -l branch_to_delete (echo {} | sed 's/^\* //' | awk '{print \$1}');
            # Use fish -c to run the confirmation and deletion command
            fish -c '
                set -l branch \"\$argv[1]\";
            ...
            ' \"\$branch_to_delete\"
        )+reload(fish -c 'source ~/.config/fish/functions/gb.fish; __gb_list_branches')+refresh-preview" \
